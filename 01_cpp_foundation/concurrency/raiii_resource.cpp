...
public:
    ...
    void set_mapper_name1(string &N){      // now N can be moved, but API took ownership , usually not intentional
        mapper_name = move(N);             
    }
    ...
    static void playground(void){
        cnt++;
        std::cout<<"playground called: "<<cnt<<" times"<<std::endl;
    }
...
int main (void){
    ...
    cout<<M1.get_mapper_name()<<endl;
    cout<<"--------------------------------"<<endl;
    M1.set_mapper_name1(s);
    cout<<M1.get_mapper_name()<<endl;
    cout<<"--------------------------------"<<endl;
    mapObject::playground();
    return 0;
}